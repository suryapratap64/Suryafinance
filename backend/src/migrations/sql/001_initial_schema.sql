...
CREATE TABLE IF NOT EXISTS securities_master (
    ...
    security_type VARCHAR(20) CHECK (
        security_type IN ('STOCK', 'MUTUAL_FUND')
    ),
    isin VARCHAR(20),
    exchange VARCHAR(10),
...
CREATE TABLE IF NOT EXISTS transactions (
    ...
    transaction_type VARCHAR(4) CHECK (
...
-- Create RLS Policies
CREATE POLICY "Everyone can read securities" ON securities_master FOR
SELECT USING (true);

CREATE POLICY "Authenticated users can insert securities" ON securities_master FOR INSERT
WITH
    CHECK (
